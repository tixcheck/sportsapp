...
export function WeightedStandingsTable({ table }: { table: WeightedTable }) {
  if (table.scoring === "placement") {
    return <PlacementStandingsTable table={table} />;
  }
...
/**
 * The same season table for a ladder scored by PLACEMENT, where a low score is
 * the good one (migration 0126).
 *
 * Mango's Fall season: each night a team scores its tier's number plus where it
 * finished — 1/2/3 in Tier 1 down to 16/17/18 in Tier 6 — and the lowest total
 * over the season wins.
 *
 * Rendered separately rather than by flipping a sort order, because almost
 * everything differs: the column is a score rather than points, sets won are
 * irrelevant, and the heading has to SAY that lowest wins. A table that looks
 * identical to the points one but ranks the other way round is how a captain
 * concludes they are top when they are bottom.
 */
function PlacementStandingsTable({ table }: { table: WeightedTable }) {
  if (table.placementRows.length === 0) return null;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Each night you score your tier&rsquo;s number plus where you finished
          — <strong>lowest total wins</strong>
          {table.placementTiers.length > 0 && (
            <>
              {" — "}
              {table.placementTiers
                .map((t) => `${t.name}: from ${t.base}`)
                .join(" · ")}
            </>
          )}
          .
      ...
      <CardContent>
          ...
          <table className="w-full text-sm tabular-nums">
            <thead>
              <tr className="text-muted-foreground border-rule border-b text-left text-xs tracking-wide uppercase">
                ...
                <th className="p-2 text-right font-semibold">Nights</th>
                <th className="p-2 font-semibold">By week</th>
              </tr>
            </thead>
            <tbody>
              {table.placementRows.map((r, i) => (
                <tr key={r.teamId} className="border-rule/60 border-b">
                  <td className="text-muted-foreground p-2">{i + 1}</td>
                  <td className="text-foreground p-2 font-medium">
                    {r.teamName}
                  ...
                  <td className="p-2">
                    <span className="flex flex-wrap gap-1.5">
                      {[...r.weeks].reverse().map((w) => (
                        <span
                          key={w.week}
                          title={`Week ${w.week} — finished ${w.rank} in ${w.tierName}`}
                          className="border-rule text-muted-foreground rounded border px-1.5 py-0.5 text-xs"
                        >
                          {w.tierName.replace(/^Tier /, "T")}{" "}
                          <span className="text-foreground font-medium">
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          {table.weeksCounted} night{table.weeksCounted === 1 ? "" : "s"}{" "}
          counted. Hover a night to see where the team finished. A night with no
          result yet is not counted.
        </p>
      </CardContent>
    </Card>
  );
