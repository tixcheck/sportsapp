...
- **Supabase project:** `evngfeuqyllfwkdvsrsb`. **Migrations written through `0066`.** From `0050` on they are **hand-written SQL** applied with a throwaway node script (drizzle-kit won't run them), so Drizzle's tracking doesn't know about any of them — see Known quirks.
